...
export const SuperAdminClientProfile = ({ 
  ...
  const handleEditStore = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget;
    setStores(stores.map((s: any) => s.id === editingStore.id ? {
      ...s,
      name: (form.elements.namedItem('name') as HTMLInputElement).value,
      address: (form.elements.namedItem('address') as HTMLInputElement).value,
      pin: (form.elements.namedItem('pin') as HTMLInputElement).value,
  ...
  const handleEditPos = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget;
    setPosMachines(posMachines.map((p: any) => p.id === editingPos.id ? {
      ...p,
      name: (form.elements.namedItem('name') as HTMLInputElement).value,
    } : p));
  ...
  const toggleStoreStatus = (storeId: number) => {
    setStores(stores.map((s: any) => s.id === storeId ? { ...s, status: s.status === 'Suspendido' ? 'Activo' : 'Suspendido' } : s));
  };

  const togglePosStatus = (posId: number) => {
    setPosMachines(posMachines.map((p: any) => p.id === posId ? { ...p, status: p.status === 'Suspendido' ? 'Activo' : 'Suspendido' } : p));
  };
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {clientStores.map((store: any) => {
            ...
            return (
              <div key={store.id} className={`bg-white rounded-2xl shadow-sm border p-6 ${store.status === 'Suspendido' ? 'border-error/30 bg-error/5' : 'border-outline-variant/20'}`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${store.status === 'Suspendido' ? 'bg-error/15 text-error' : 'bg-green-100 text-green-700'}`}>
                        {store.status || 'Activo'}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      title={store.status === 'Suspendido' ? 'Activar local' : 'Suspender local'}
                      className={`p-2 rounded-lg transition-colors ${store.status === 'Suspendido' ? 'text-green-600 hover:bg-green-100' : 'text-error hover:bg-error/10'}`}
                    >
                      {store.status === 'Suspendido' ? <CheckCircle className="w-4 h-4" /> : <MinusCircle className="w-4 h-4" />}
                    </button>
                    <button
                ...
                <div className="space-y-2 mt-4">
                  ...
                  {storePos.length === 0 ? (
                    ...
                    storePos.map((pos: any) => (
                      <div key={pos.id} className={`flex items-center justify-between p-3 rounded-xl border ${pos.status === 'Suspendido' ? 'bg-error/5 border-error/20' : 'bg-surface-container-lowest border-outline-variant/10'}`}>
                        <div className="flex items-center gap-3">
                          <Monitor className={`w-4 h-4 ${pos.status === 'Suspendido' ? 'text-error' : 'text-on-surface-variant'}`} />
                          <span className="font-bold text-sm text-on-surface">{pos.name}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`text-xs font-bold px-2 py-1 rounded-full ${pos.status === 'Suspendido' ? 'bg-error/15 text-error' : 'bg-green-100 text-green-600'}`}>
                            {pos.status || 'Activa'}
                          </span>
                          <button onClick={() => { setEditingPos(pos); setShowEditPosModal(true); }} title="Editar caja" className="p-1.5 text-on-surface-variant hover:text-secondary hover:bg-secondary/10 rounded-lg transition-colors">
                            <Edit className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => togglePosStatus(pos.id)} title={pos.status === 'Suspendido' ? 'Activar caja' : 'Suspender caja'} className={`p-1.5 rounded-lg transition-colors ${pos.status === 'Suspendido' ? 'text-green-600 hover:bg-green-100' : 'text-error hover:bg-error/10'}`}>
                            {pos.status === 'Suspendido' ? <CheckCircle className="w-3.5 h-3.5" /> : <MinusCircle className="w-3.5 h-3.5" />}
                          </button>
                        </div>
                      </div>
                    ))
      ...
      <AnimatePresence>
        ...
        {showEditStoreModal && editingStore && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-surface w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-white/20">
              <div className="p-6 border-b border-outline-variant/20 flex justify-between items-center bg-surface-container-lowest">
                <h3 className="text-xl font-black font-headline">Editar Local</h3>
                <button onClick={() => { setShowEditStoreModal(false); setEditingStore(null); }} className="p-2 hover:bg-surface-container-high rounded-full transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={handleEditStore} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-bold text-on-surface-variant mb-1">Nombre del Local</label>
                  <input name="name" required defaultValue={editingStore.name} className="w-full p-3 bg-surface-container-lowest border border-outline-variant/30 rounded-xl focus:ring-2 focus:ring-secondary outline-none font-medium" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-on-surface-variant mb-1">Dirección</label>
                  <input name="address" required defaultValue={editingStore.address} className="w-full p-3 bg-surface-container-lowest border border-outline-variant/30 rounded-xl focus:ring-2 focus:ring-secondary outline-none font-medium" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-on-surface-variant mb-1">PIN de Acceso (4 dígitos)</label>
                  <input name="pin" required pattern="\\d{4}" maxLength={4} defaultValue={editingStore.pin} className="w-full p-3 bg-surface-container-lowest border border-outline-variant/30 rounded-xl focus:ring-2 focus:ring-secondary outline-none font-medium font-mono" />
                </div>
                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => { setShowEditStoreModal(false); setEditingStore(null); }} className="flex-1 py-3 px-4 bg-surface-container-high hover:bg-surface-container-highest text-on-surface rounded-xl font-bold transition-colors">Cancelar</button>
                  <button type="submit" className="flex-1 py-3 px-4 bg-secondary hover:bg-secondary/90 text-white rounded-xl font-bold transition-colors shadow-md">Guardar Cambios</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
        ...
        {/* Edit POS Modal */}
        {showEditPosModal && editingPos && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-surface w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-white/20">
              <div className="p-6 border-b border-outline-variant/20 flex justify-between items-center bg-surface-container-lowest">
                <h3 className="text-xl font-black font-headline">Editar Caja</h3>
                <button onClick={() => { setShowEditPosModal(false); setEditingPos(null); }} className="p-2 hover:bg-surface-container-high rounded-full transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={handleEditPos} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-bold text-on-surface-variant mb-1">Nombre de la Caja</label>
                  <input name="name" required defaultValue={editingPos.name} className="w-full p-3 bg-surface-container-lowest border border-outline-variant/30 rounded-xl focus:ring-2 focus:ring-secondary outline-none font-medium" />
                </div>
                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => { setShowEditPosModal(false); setEditingPos(null); }} className="flex-1 py-3 px-4 bg-surface-container-high hover:bg-surface-container-highest text-on-surface rounded-xl font-bold transition-colors">Cancelar</button>
                  <button type="submit" className="flex-1 py-3 px-4 bg-primary hover:bg-primary/90 text-white rounded-xl font-bold transition-colors shadow-md">Guardar Cambios</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
